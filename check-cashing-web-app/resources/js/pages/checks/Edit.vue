...
import { type CheckForm, type Check } from '@/types';
...
const { currentCheck } = defineProps<{
    currentCheck: Check,
}>();

// Customer form parameters
const form = useForm<CheckForm>({
    company_name: currentCheck.company_name,
    company_address_street: currentCheck.company_address_street,
    company_address_city: currentCheck.company_address_city,
    company_address_state: currentCheck.company_address_state,
    company_address_zip: currentCheck.company_address_zip,
    account_number: currentCheck.account_number,
    routing_number: currentCheck.routing_number,
    type: currentCheck.type,
    cashing_status: currentCheck.cashing_status,
...
// Submit form action
const submit = () => {
    form.post(route('checks.update', currentCheck.id));
};
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 items-center">
            <form @submit.prevent="submit" class="flex flex-col gap-6 w-3/5 max-w-4xl overflow-auto">
                <Card class="min-w-full">
                    <CardHeader>
                    <CardTitle>Edit Check</CardTitle>
                    </CardHeader>
                    <CardContent>
