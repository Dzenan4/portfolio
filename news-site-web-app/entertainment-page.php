...
<body class="body-container">
    ...
    <div class="header-container">
        ...
        <div class="nav-bar">
            ...
            <div class="logout-options">
                ...
                if ($_SESSION['guest']) {
                    echo "<form action='login-page.php' method='post'>
                            <input class='logout-btn' type='submit' name='go-login' value='Login'>
                            <input type='hidden' name='token' value='<?php echo " . $_SESSION['token'] . ";?'>
                            </form>";
                } else {
                    printf("<form method='POST'><label class='username'>%s</label>
    ...
    <!-- A list of stories with a genre of entertainment -->
    <div>
        <h2 class='body-title'>Entertainment News</h2><br>
        <?php
        ...
        $stmt = $mysqli->prepare("select story_id, title, body, link, genre, DATE_FORMAT(publish_date, '%m-%d-%Y'), users.username from user_stories 
                                    left join users on user_stories.user_id = users.user_id where genre='Entertainment' order by publish_date desc limit 15");
